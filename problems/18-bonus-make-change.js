...
function makeChange(amount, coins = [1, 5, 10, 25]) {
  if (amount === 0) {
    return [];
  }
  for (let i = coins.length - 1; i >= 0; i--) {
    ...
    if (coin <= amount) {
      ...
      const change = makeChange(remainder, coins);
      if (change !== null) {
        return [...change, coin];
      }
    }
...
function makeBetterChange(amount, coins = [1, 5, 10, 25]) {
  ...
  for (let i = 0; i < coins.length; i++) {
    ...
    if (coin <= amount) {
      const remainder = amount - coin;
      const change = makeBetterChange(remainder, coins.slice(i));
      if (change !== null) {
        const currentChange = [coin, ...change];
